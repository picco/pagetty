...
Pagetty.createSegments = function(links, page, callback) {
  ...
  for (var i in links) {
    if (path = this.getSegmentPath(links[i])) {
  ...
  for (var i in paths) {
    ...
    var weight = 0;
    var commonSelector = this.getCommonSelector(page.find(selector).toArray());
    ...
    if (_.uniq(titles).length > 3 && onlyNumbers < 3) {
      segments.push({path: paths[i], commonSelector: commonSelector, links: la, weight: weight});
    }
  }
 ...
 */
Pagetty.getSegmentPath = function(el, path) {
  ...
  // Recurse up the DOM.
  return this.getSegmentPath(el.parent(), cur + (path ? (" > " + path) : ""));
}
...
Pagetty.getCommonSelector = function(links) {
  var elements = [];
  var allClasses = {};
  var usableClasses = [];
  var commonAncestor = this.findCommonAncestor(links);
  var tagNames = [];

  for (var i in links) {
    elements.push(xxx($(links[i]), commonAncestor));
  }
