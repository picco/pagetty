...
Pagetty.updateSubscription = function(user_id, channel_id, data, callback) {
  ...
  console.dir(query);
  this.users.update({_id: new ObjectID(user_id)}, {$set: query}, {safe: true}, function(err) {
    callback(err);
  });
}
...
Pagetty.updateChannelItems = function(channel_id, callback) {
  ...
  this.channels.find(query).each(function(err, channel) {
    ...
    if (channel) {
      ...
      logger.log.error("Channel " + channel_id + " not found for updating.");
      callback();
    }
  });
}
...
Pagetty.fetchData = function(options) {
  return new $.Deferred(function(dfd) {
    ...
    if (options.url == null || !options.url.match(/^(http|https):\/\//)) {
      logger.log.error("Pagetty.fetchData: Invalid URL");
...
Pagetty.subscribe = function(options, callback) {
  ...
  sequence.then(function(next) {
    ...
    self.channels.findOne({url: options.url}, function(err, doc) {
      if (doc) {
        ...
        self.channels.insert({url: options.url, domain: urlComponents.host}, function(err, docs) {
          channel = docs[0];
          next();
        });
      }
  ...
  .then(function(next) {
    ...
    var query = {}; query["subscriptions." + channel._id] = {name: "noname"};
