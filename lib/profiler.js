 ...
 * TODO
 */
Profiler.createSegments = function(url, page, callback) {
  var self = this;
  var segments = [];
  var linkSegments = [];
  var itemSegments = [];
 ...
 // Build the itemSegments object.
  ...
  for (var i in segments) {
    ...
    for (var j in els) {
      var title = segment.title.attribute ? $(els[j]).attr(segment.title.attribute) : $(els[j]).text();
      var href = require('url').resolve(url, encodeURI($(els[j]).attr(segment.target.attribute))) || null;
    ...
    if (segment.combinedSelector.match(/h1|h2|article/)) segment.weight += 30;
    if (segment.combinedSelector.match(/h3|h4/)) segment.weight += 20;
...
Profiler.findTarget = function(segment, page, itemSelector) {
  ...
  segment.target = {selector: '', attribute: 'href'};
  segment.title = {selector: '', attribute: ''};
  ...
  segment.target.selector = segment.title.selector = this.getSelector(this.getSegmentPath(this.findBestMatch(candidates)));
...
Profiler.findImage = function(segment, page, itemSelector) {
  ...
  segment.image = {selector: 'img', attribute: 'src'};
...
Profiler.findCommentsScore = function(segment, page, itemSelector) {
  ...
  segment.comments = {selector: '', attribute: ''};
  ...
  if (segment.comments.selector) segment.comments.attribute = 'href';
}
