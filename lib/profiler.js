...
Profiler.createProfile = function(page, callback) {
  ...
  for (var segmentSelector in segments) {
    ...
    if (!segment.itemSelector) {
      continue;
    }
    
    segment.targetSelector = self.findTargetSelector(page, segment.itemSelector);        
...
Profiler.getItemSelector = function(rootElement) {
  //return this.getSelector(this.getPath(rootElement));
 ...
 * TODO
 */
Profiler.findTargetSelector = function(page, itemSelector) {
  var selector = {selector: null, url_attribute: 'href', title_attribute: ''};
  var candidates = [];
  var sample = $(itemSelector, page).html();  
  ...
  for (var i in links) {
    ...
    if (s.match(/ h5| h6|\.article|\.title|\.post|\.newsitem/)) w += 10;
    
    candidates.push({p: p, w: w});
  }

  selector.selector = this.findBestMatch(candidates); 
  console.dir('Best: ' + selector.selector);
  return selector;
}
...
Profiler.findBestMatch = function(candidates) {
  ...
  return this.getSelector(this.getSegmentPath(candidates.shift().p));
}
