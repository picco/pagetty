...
Parser.process = function(html, channel, parser, callback) {
  var self = this;
  ...
  this.handler = new htmlparser.DefaultHandler(function (err, dom) {
    if (err) {
      ...
      for (i in parser.rules) {
        var elements = select(dom, parser.rules[i].item);
        for (j in elements) {
          var element = elements[j];
          var item = self.processItem(element, channel, parser.rules[i]);
...
process.on('message', function(m) {
  Parser.process(m.html, m.channel, m.parser, function(items) {
    process.send(items);
    process.exit();
  });
});