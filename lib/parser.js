 ...
 */
Parser.processRSS = function(baseURL, articles, callback) {
  var self = this;
  var items = [];

  async.forEach(articles, function(article, next) {
    items.push(self.processItem({baseURL: baseURL, rssItem: article}));
    next();
...
Parser.processItem = function(options) {
  ...
  if (options.htmlItem) {
    ...
    if (options.rssItem.image.url) {
      var image = self.processURL(options.baseURL, options.rssItem.image.url);
    }
    else if (options.rssItem.enclosures && options.rssItem.enclosures[0] && options.rssItem.enclosures[0].type == 'image/jpeg') {
      var image = self.processURL(options.baseURL, options.rssItem.enclosures[0].url);
    }
    else {
      var image = self.processURL(options.baseURL, self.processElement(options.rssItem.description, 'img', 'src'));
    }
