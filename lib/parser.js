...
Parser.processItem = function(baseURL, raw, rule) {
  ...
  var item = {
    title: self.processTitle(self.processElement(raw, rule.title.selector, rule.title.attribute)),
    target: self.processURL(baseURL, self.processElement(raw, rule.target.selector, rule.target.attribute)),
